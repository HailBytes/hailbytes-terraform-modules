...
| `vm` | A third Linux VM, Ubuntu 24.04 + apt-installed PostgreSQL 16 on a Premium_LRS data disk | No automated backups, no PITR, no zone-redundant failover. Pre-patch protection is a managed-disk snapshot only, and restore means attaching a snapshot to a new VM. Does **not** carry the per-vCPU Marketplace meter: it boots a Canonical Ubuntu image with no `plan {}` block, and the meter attaches to the HailBytes Marketplace plan that only the app VMs reference — see [`AZURE_COST_SHAPES.md`](../AZURE_COST_SHAPES.md). |
