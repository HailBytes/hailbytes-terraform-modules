...
The Azure rows above are superseded. `vm_size` moved off Dsv5 to
`Standard_B4ms` (4 vCPU, burstable); AWS is unchanged on `m6i.2xlarge`. (This
revision is itself superseded by 2026-09-09 below, which moves off B-series;
the reasoning here still explains why Dsv5 could not stay the default.)
...
An HA pair at the then-current default needed **16** vCPUs of that pool. And the
...
### Revised 2026-09-09: B-series is gone, and the HA default is 2 vCPU

The `Standard_B4ms` rows below are superseded again. Current Azure defaults,
read from `variables.tf` rather than from memory:

| Tier module | Default | Instances | Metered vCores | Matching SKU |
|---|---|---:|---:|---|
| `single-vm/azure` | `Standard_D4s_v5` (4) | 1 | **4** | none — below `HB-ESS` (8) |
| `ha-hot-hot/azure` | `Standard_D2s_v3` (2) | 2 | **4** | none — below `HB-ESS` (8) |
| `unlimited-scale/azure` | `Standard_D4s_v5` (4) | 2 | **8** | `HB-ESS` (8) |

Two changes since the 2026-09-07 revision:

**B-series is no longer any tier's default.** It had the deploys-first-time
property, but it is **burstable**: it banks CPU credits while idle and throttles
to a fraction of a core once they are spent, which degrades campaign sending
with no signal at all. `Standard_D2s_v3` gets the same quota property without
the throttle, because `standardDSv3Family` is granted a non-zero default on
most subscriptions where `standardDSv5Family` sits at 0.

**`ha-hot-hot/azure` dropped to 2 vCPU, and that costs a sellable rung.** It
now meters 4 vCores across the pair, so HA has joined `single-vm` in matching
no purchasable SKU. That is a real regression against the price list and it was
accepted knowingly: a default that deploys on the first attempt in a
subscription that has never requested quota was judged worth more than a
default that matches the ladder, because the alternative kept costing live
deployment calls. Only `unlimited-scale/azure` still lands on `HB-ESS`.

**ASM is unaffected.** `asm-azure-ha` carries its own `Standard_D4s_v5`
default rather than inheriting the tier's, because
`hailbytes-asm/docs/HARDENING_GUIDE.md` sets a 4 vCPU minimum and a 2 vCPU
default would ship ASM below its own documented floor.

`Standard_D8s_v5` remains on the ladder and remains the published purchasable
rung; the upgrade is a `vm_size` change once Dsv5 quota is granted.

Revisit if Azure's default family grants change, or if the price list gains a
rung below 8 vCores.
