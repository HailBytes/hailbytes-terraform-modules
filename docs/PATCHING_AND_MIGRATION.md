...
|---|---|---|
| **Patches are customer-initiated.** | Module ships a Systems Manager Run Command document (`<prefix>-pre-patch-backup`) the customer fires from the AWS Console. HailBytes never has credentials. | Module ships `azurerm_virtual_machine_run_command` `RunPrePatchBackup`. **Note the semantic difference from AWS** (gap C2): an SSM document is inert until invoked, whereas an Azure Run Command resource *executes its script body when Terraform creates it*, and then persists on the VM as a re-runnable command. So the first execution happens during `terraform apply`, not at the customer's first patch window — the Portal blade (VM → Operations → Run command) re-runs it thereafter. HailBytes never has credentials either way. |
