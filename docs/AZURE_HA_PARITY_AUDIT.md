...
| `lifecycle { ignore_changes = [ami, user_data] }` on the instances | `lifecycle { ignore_changes = [source_image_reference, custom_data] }` on `azurerm_linux_virtual_machine.vm` and `.db_vm` | ✅ **real** | Gap B5, fixed. Was: no `lifecycle` block at all, so a published image version replaced **both** VMs on any apply. Now symmetric with AWS. |
...
| **B1** | **No flow logs.** `enable_flow_logs` is AWS-only; `SECURITY-DEFAULTS.md` states Azure NSG flow logs are on by default. | P1 | `grep -rln enable_flow_logs modules/` → AWS modules only. | Add VNet flow logs (`azurerm_network_watcher_flow_log`) + a Storage Account destination, gated on `enable_flow_logs` for symmetry. Note NSG flow logs are being retired in favour of VNet flow logs — build the VNet variant. | ✅ **FIXED** — `azurerm_network_watcher_flow_log` on the vnet, behind `enable_flow_logs` (default `true`, matching AWS), to a Storage Account with `shared_access_key_enabled = false`. Built as **VNet** flow logs rather than NSG flow logs because NSG flow logs are being retired. Needs a Network Watcher in the region; defaults to Azure's auto-created `NetworkWatcher_<region>` in `NetworkWatcherRG`. |
| **B2** | **No load-balancer or gateway access logs, and no diagnostic settings at all.** AWS ships optional ALB access logs to a versioned, lifecycled bucket. Azure ships nothing, and `SECURITY-DEFAULTS.md` claims a Storage Account with a 90-day lifecycle. | P1 | No `azurerm_monitor_diagnostic_setting` in any Azure module. | Add diagnostic settings for the LB / App Gateway (and ideally Flexible Server + Redis) to a Storage Account or Log Analytics workspace, behind one variable. | ✅ **FIXED** — `azurerm_monitor_diagnostic_setting` for the load balancer, Flexible Server, Redis and (when enabled) Application Gateway, to a Log Analytics workspace created by the module or supplied via `log_analytics_workspace_id`. One honest caveat now documented rather than glossed: a Standard Load Balancer is L4 and has **no** request-level access log, so `ApplicationGatewayAccessLog` is the only real analogue of AWS ALB access logs and it requires the App Gateway. |
...
| **C2** | **`azurerm_virtual_machine_run_command` is not an on-demand document.** The script body executes when Terraform *creates* the resource. `PATCHING_AND_MIGRATION.md` describes both Run Commands as documents "the customer fires from the Portal", which mis-describes the resource. | P2 | `azurerm_virtual_machine_run_command` semantics; `docs/PATCHING_AND_MIGRATION.md` row 1 and step 2. | Correct the doc (done for Azure in the new runbook), and consider shipping the scripts in the image only, with a documented `az vm run-command invoke` instead of a persistent resource. | ✅ **FIXED (documentation)** — `PATCHING_AND_MIGRATION.md` row 1 now states the difference explicitly: an SSM document is inert until invoked, an Azure Run Command resource executes its script body when Terraform *creates* it and then persists as a re-runnable command. The script is not moved out of the resource: executing the pre-patch backup once at apply time is harmless (it produces a bundle nobody needs yet) and having the command persisted on the VM is what makes the Portal blade work, which is the interface the runbook tells operators to use. **No code change** — the resource was right, the sentence describing it was wrong. |
| **C3** | **Tests never exercise North Europe, zone placement, or IAM wiring.** Both Azure HA test files use `location = "eastus"`. No assertion covers zone assignment, the Key Vault role assignments, Redis reachability, or `db_mode = "vm"`. `create_backup_storage_account` is forced `false` in both suites, so the backup path is untested. | P2 | `modules/ha-hot-hot/azure/tests/*.tftest.hcl`. | Add `northeurope` cases, assert `vm[*].zone == ["1","2"]`, assert one Key Vault Secrets User role assignment per VM (regression test for A1), and add a `db_mode = "vm"` plan case. | ✅ **FIXED** — new `tests/northeurope.tftest.hcl`: 7 runs in `northeurope`, covering distinct VM zones, ZoneRedundant DB, a zone-redundant frontend IP, one Key Vault Secrets User assignment per VM (the A1 regression), the Redis key secret, the whole backup path with `create_backup_storage_account = true` (immutability policy, no shared-key auth, no public access, one writer role per VM), `db_mode = "vm"` end to end, and the C4/C5 additions. Suite went 19 → 26 runs. Planning `db_mode = "vm"` for the first time surfaced that the mock provider returns an *empty* `address_prefixes` for `data.azurerm_subnet` — impossible for a real subnet — so the mock now supplies one; the module reads that CIDR to scope the db VM's NSG rule and its `pg_hba` entry. |
| **C4** | **Flexible Server observability and safety knobs missing.** No `log_min_duration_statement` equivalent (the AWS parameter group sets it), no Performance-Insights analogue, no `deletion_protection` equivalent. | P2 | `modules/ha-hot-hot/aws/main.tf` `aws_db_parameter_group` + `rds_performance_insights_*` vs the Azure `azurerm_postgresql_flexible_server_configuration` (which only sets `require_secure_transport`). | Add server configurations for slow-query logging; consider `azurerm_management_lock` for delete protection. | ✅ **FIXED** — `log_min_duration_statement` via `azurerm_postgresql_flexible_server_configuration`, default 1000 ms to match the AWS parameter group, behind `db_log_min_duration_ms`. This mattered more than it looks: Flexible Server's default is `-1`, so the `PostgreSQLLogs` diagnostic setting added for B2 was shipping logs with no statement durations in them. Delete protection is `enable_db_delete_lock` → a `CanNotDelete` management lock scoped to the server, **off by default** precisely because it works — a locked server makes `terraform destroy` fail partway and leave a half-destroyed stack, which is a worse PoC experience than an accidental delete. Turn it on for production; disable it in a separate apply before a planned teardown. |
| **C5** | **Key Vault name collision after a destroy.** `purge_protection_enabled = true` with `soft_delete_retention_days = 30` and a name derived from `name_prefix`: destroying and re-creating the same-named stack inside 30 days fails, and purge protection means it cannot be force-purged. A real footgun during PoC iteration. | P2 | `modules/ha-hot-hot/azure/main.tf` `azurerm_key_vault.main`. | Document it prominently, or add a short random suffix to the vault name. Do **not** disable purge protection — disk encryption sets require it. | ✅ **FIXED** — new `key_vault_name` variable (both Azure tiers + all four wrappers), defaulting to `null` = the existing derived name. A random suffix was considered and rejected: it would change the vault name on every existing deployment, and a Key Vault rename is a *replacement* — destroying the vault that holds the DB password and Redis key, and taking the disk encryption set with it. An opt-in override is non-destructive for existing state and gives PoC iteration a way out. Purge protection stays on, as the audit required. |
...
|---|---|---|
| Does App Gateway v2 accept a self-signed backend certificate? | **No.** A self-signed or unknown-CA backend requires its root uploaded as a trusted root certificate on the backend settings, *and* the backend `host` must match the certificate CN. Otherwise the gateway marks the pool unhealthy. Confirms A6 was a real defect, now fixed. | [Application Gateway end-to-end TLS](https://learn.microsoft.com/en-us/azure/application-gateway/ssl-overview) |
...
3. Whether the marketplace offer deploys in North Europe at all (C6).
4. Whether `azurerm_virtual_machine_run_command` failing at apply time fails the
   whole `terraform apply` or merely records a non-zero exit.
5. Whether the Redis private endpoint resolves through the private DNS zone from
   the VM subnet on first boot.
