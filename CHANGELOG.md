...
- **`quickstart/sweep-azure.sh`: find and clean up the leftovers from repeated test deploys.** Debris from a failed or abandoned apply is not inert — a half-built load balancer, an orphaned diagnostic setting or a pre-existing key vault makes the *next* apply stop partway through with `already exists - to be managed via Terraform this resource needs to be imported`, after other resources have already been created, so the failed apply leaves debris of its own. Until now finding that meant a hand-written `az` loop per group, and the child resources most likely to be responsible are invisible to `az resource list`.
  ...
  `list` / `show` / `imports` change nothing; only `delete` does. `show` reports the child resources `az resource list` omits. `imports` **prints** `terraform import` commands for a human to read and never runs one — for test debris, deleting is almost always right and importing almost never is, because `terraform import` writes ownership into state and a resource whose settings don't exactly match the configuration is then mutated by the next apply and deleted by the next destroy. When it does emit them it includes the load balancer's children (pool, probes, rules), since importing the load balancer alone just moves the same failure one layer down.

  `delete` refuses outright when the hostname passed to `--hostname` currently resolves to an address inside the group: Azure has **no undelete for a public IP**, so deleting the group loses the address permanently and forces a DNS change. Locks, storage accounts and unattached public IPs are reported before it asks for the group name to be typed. There is no `--force`.

  A failed lookup is never rendered as an empty result: a filter that matches nothing says so and lists what does exist, and an unreadable subscription exits non-zero with the CLI's own error rather than reporting "nothing to clean up" — the answer that would be most costly to believe.
...
- **CI shellcheck covered 5 of the repo's 10 shell scripts.** The step listed paths by hand, so `keyvault-maintenance.sh`, `check-sender-reputation.sh`, both `cloudshell.sh` files — and any script added later — were never linted. It now `find`s every `*.sh` and passes `-x` to follow `source`. All 10 pass at `-S warning` today, so widening the gate turns nothing red; the point is that the next script cannot silently escape it.
...
- **`b111065`'s storage-safety fix reached nobody: it flipped the default on the three Azure core modules and left the old value on all six Azure wrappers.** That change set `create_backup_storage_account = false` to stop an apply from outside the vnet failing `403 KeyBasedAuthenticationNotPermitted`. The cores got `false`. `sat-azure-ha`, `asm-azure-ha`, `sat-azure-single`, `asm-azure-single`, `sat-azure-autoscale` and `asm-azure-autoscale` kept `true` — and because a wrapper forwards its own value into the core, `true` won for **every caller of the product modules, which are the public API**. The fix was inert where it mattered.
  ...
  **The check that was supposed to catch this did catch it, and could not stop it.** `wrapper_drift_check.py` printed six warnings reading `create_backup_storage_account default differs from core: core='false' wrapper='true'`, and the step was non-blocking, so the build stayed green. It now **fails** on a drifted `type` or `default` — both of which change behaviour, since the wrapper's value is the one that reaches the core — and keeps `description` drift as a warning, because that is documentation. Today that is 0 errors and 43 description warnings.

  **Upgrade impact:** callers who relied on the wrapper default to get a backup Storage Account will no longer get one and must set `create_backup_storage_account = true` explicitly — which is the same position core-module callers have been in since `b111065`, and is deliberate: that account cannot currently be created from outside the vnet. Nothing is destroyed by the default change alone; an existing account stays in state and keeps being managed while the value is `true` in your own configuration.
