...
- **azurerm capped at `< 5.0`.** The modules use `enable_rbac_authorization` and the storage container `resource_manager_id`, which azurerm 4.x deprecates and 5.x removes; their replacements don't exist in 3.x. The cap keeps the documented `>= 3.0` floor working until a coordinated `>= 4.0` floor bump.
