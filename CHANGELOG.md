...
- **Removed 9 dead `#tfsec:ignore` annotations** left over across `single-vm/aws`, `unlimited-scale/{aws,azure}`, and `ha-hot-hot/{aws,azure}` from before the repo migrated to checkov + trivy-iac (see the `docs: replace stale tfsec references with checkov/trivy-iac` commit). No CI job has read these annotations since that migration, so they suppressed nothing — a contributor auditing a finding could mistake them for a live, working suppression. The underlying findings are already covered where applicable in `.checkov.yaml` (`CKV_AZURE_109`/`189`, `CKV_AWS_290`/`355`, `CKV_AWS_145`); the surrounding rationale comments are kept. No resource or behavior changes.
