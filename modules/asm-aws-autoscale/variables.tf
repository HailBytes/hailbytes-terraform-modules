# Variables for ASM on AWS (autoscale)
...
variable "enable_http_redirect" {
  description = "Add an HTTP:80 listener that 301-redirects to HTTPS. Convenient when operators hit the bare hostname. ASM only, and ignored on SAT: there, port 80 is the phishing/landing frontend, and redirecting a target who clicked a phishing link to the admin console would break the simulation."
  type        = bool
  default     = true
}
...
variable "phish_port" {
  description = "Port the phishing/tracking server listens on inside each instance. SAT only -- it is the landing-page and interaction-tracking surface, which is the product. The ALB fronts it on port 80 and forwards to this port; ignored when `product` is \"asm\", which has no phishing surface. Leave null for the image default of 80."
  type        = number
  default     = null
}
