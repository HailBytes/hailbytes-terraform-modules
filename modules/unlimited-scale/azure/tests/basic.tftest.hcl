# Minimal-input apply against a mocked azurerm provider. Proves the autoscale
# tier instantiates with only its required variables and that the scale set,
# load balancer, database, Key Vault and Redis outputs are all populated.
#
...
mock_provider "azurerm" {
  ...
  # resource IDs and validates client_config.tenant_id as a UUID; the mock
  # provider would otherwise fill these with short random tokens.
