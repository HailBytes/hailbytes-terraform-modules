# Minimal-input apply against a mocked AWS provider. Proves the autoscale tier
# instantiates with only its required variables and that the load-balancer,
# autoscaling-group, database and Redis outputs are all populated.
#
...
mock_provider "aws" {
  ...
  # mock provider fills computed strings with short random tokens.
