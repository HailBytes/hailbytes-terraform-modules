...
HailBytes ASM (Attack Surface Management) deployed on Azure — **Single VM** tier.

This module is a thin wrapper around [`modules/single-vm/azure`](../single-vm/azure) with `product = "asm"` hardcoded. All other variables, defaults, outputs, security posture, and marketplace lookup logic come from the inner tier module — see its README for the architecture diagram, cost estimate, prerequisites, and detailed inputs.

## Usage

```hcl
module "hailbytes_asm" {
  source = "github.com/hailbytes/hailbytes-terraform-modules//modules/asm-azure-single?ref=v1.0.0"

  # ... see modules/single-vm/azure/README.md for required and optional variables
}
```
