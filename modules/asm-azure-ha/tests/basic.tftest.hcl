...
# ha-hot-hot/azure output, and that `product` is correctly hardcoded to "asm".
#
...
run "wrapper_outputs_populated" {
  ...
  assert {
    ...
    error_message = "redis_endpoint must be empty by default - enable_managed_redis defaults to false on this wrapper and its core, so a non-empty endpoint means a cache was created unasked."
  }
  ...
  assert {
    condition     = output.post_patch_run_command_name != ""
    error_message = "post_patch_run_command_name output must be re-exported"
  }
}
