...
HailBytes ASM (Attack Surface Management) deployed on AWS — **Single VM** tier.

This module is a thin wrapper around [`modules/single-vm/aws`](../single-vm/aws) with `product = "asm"` hardcoded. All other variables, defaults, outputs, security posture, and marketplace lookup logic come from the inner tier module — see its README for the architecture diagram, cost estimate, prerequisites, and detailed inputs.
