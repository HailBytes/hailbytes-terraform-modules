...
# Postgres-delegated db subnet), a private DNS zone for Postgres Flexible Server,
# and a DNS zone virtual-network link. All resources are unconditional, so a
# single apply run covers the full module surface.
