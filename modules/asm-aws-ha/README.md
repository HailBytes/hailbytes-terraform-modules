...
HailBytes ASM (Attack Surface Management) deployed on AWS — **HA hot-hot (2x active/active behind a load balancer with managed Postgres)** tier.

This module is a thin wrapper around [`modules/ha-hot-hot/aws`](../ha-hot-hot/aws) with `product = "asm"` hardcoded. All other variables, defaults, outputs, security posture, and marketplace lookup logic come from the inner tier module — see its README for the architecture diagram, cost estimate, prerequisites, and detailed inputs.
...
module "hailbytes_asm" {
  source = "github.com/hailbytes/hailbytes-terraform-modules//modules/asm-aws-ha?ref=v1.0.0"
