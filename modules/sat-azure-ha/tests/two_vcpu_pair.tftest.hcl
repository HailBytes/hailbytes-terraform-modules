...
variables {
  ...
  # Admin console: a v4 office range and a v6 range, as a real tenant supplies them.
  allowed_cidrs = ["203.0.113.0/24", "2001:db8:2000:1870::/64"]
  # Landing pages have to be reachable by targets, so they are allow-listed apart.
...
run "two_vcpu_pair_plans_and_meters_four_vcores" {
  command = apply

  assert {
    condition     = length(output.vm_ids) == 2
    error_message = "The 2-vCPU pilot must still be a PAIR. One node is not HA, and it halves the metered vCore count a customer was quoted on."
  }

  assert {
    condition     = output.load_balancer_public_ip != ""
    error_message = "load_balancer_public_ip must be non-empty at the 2-vCPU rung, same as at 8"
  }

  assert {
    condition     = output.postgres_fqdn != ""
    error_message = "The database is a Flexible Server at every rung; it carries no marketplace image and draws no meter"
  }

  assert {
    condition     = output.vm_nsg_id != ""
    error_message = "vm_nsg_id must be non-empty — it is what carries the split admin/phishing allow-lists"
  }
}
